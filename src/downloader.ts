...
async function runDownload(
  ...
  // 是否已原子提交到最终输出（决定失败时是否清理临时文件）
  // whether the output was atomically committed (drives failure cleanup)
  let committed = false;
  ...
  try {
    ...
    // 未成功提交（下载失败 / 合并失败 / 校验失败）→ 清理分块/单流临时文件，
    // 仅保留 merged（供断点续传）。成功路径已 removeAllTempFiles。
    // If not committed (download/merge/checksum failure): remove chunk &
    // single-stream temp files, keep merged for resume.
    if (!committed) removeNonMergedTempFiles(outPath);
  }
}
